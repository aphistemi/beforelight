...
export default function VideoSection({
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const shouldFocus =
        distance < threshold &&
        containerRect.top < viewportCenter &&
        containerRect.bottom > viewportCenter;
  ...
  const handlePlayClick = async () => {
    ...
    try {
      if (isPlaying) {
        ...
        if (isIOS) {
          // iOS: adjust properties before playing
          vid.muted = false;
          vid.preload = "auto";
          setIsMuted(false);

          // Wait until iOS confirms it's ready
          await new Promise((resolve) => {
            if (vid.readyState >= 3) {
              resolve(true);
            } else {
              vid.addEventListener("canplay", () => resolve(true), {
                once: true,
              });
            }
          });
      ...
      if (error.name === "AbortError") {
        console.debug("Play aborted (can ignore):", error);
      } else {
            ...
            <div className="absolute inset-0 bg-black/20 flex items-center justify-center group-hover:bg-black/30 transition-all duration-300 pointer-events-none">
              <div className="text-center">
                ...
                <div className="text-white/95">
                  <div className="text-base md:text-lg font-light tracking-[0.3em] mb-1">
                    PLAY
                  </div>
